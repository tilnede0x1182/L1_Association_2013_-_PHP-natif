<?php session_start();
	...
	$_SESSION['pageCourante']=$serveur."Contenu/Posts/ListedesPosts.php";
	...
	if (verifieConnection()){
			...
			echo "Pas de connexion au serveur" ;
				...
				if (!$connexion) {
					...
					echo '<h1>Liste des articles publiés par le membre <a href="'.$serveur.'Membres/InformationMembre.php?idmembre='.$membre.'">'.$membre.'</a> : </h1>'."\n";
	  ...
	  <th>Modifiaction</th>
					...
					while (true) {
						...
						if ($ligne==false){
							...
							echo '	  <td><h4><a href="'.$serveur.'Contenu/Posts/AfficheArticle.php?idpost='.$idpost.'">'.$ligne['Objet'].'</a></h4>'.nl2br(detectlId ($ligne['Post']), false).'</td>'."\n";
							echo '	  <td>'.convertDate($ligne['date']).'</td>'."\n";
							echo '	  <td><div class="lien"><a href="'.$serveur.'Contenu/Posts/ModifierArticle.php?idarticle='.$idpost.'">modifier</a></div><br><br><br>'."\n".'<div class="lien"><a href="'.$serveur.'Contenu/Posts/SupprimerArticle.php?idpost='.$idpost.'"">supprimer</a></div></td>';
							echo '	 </tr>'."\n";
						}
					}
